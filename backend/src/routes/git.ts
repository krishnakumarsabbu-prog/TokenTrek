...
router.get('/stats', (_req, res) => {
  const scrum = store.scrum_records || [];
  const kanban = store.kanban_records || [];
  ...
  // Group scrum by team
  const teamsScrumMap: Record<string, any[]> = {};
  scrum.forEach(r => {
    if (!teamsScrumMap[r.team_name]) teamsScrumMap[r.team_name] = [];
  ...
  const teamsKanbanMap: Record<string, any[]> = {};
  kanban.forEach(r => {
    if (!teamsKanbanMap[r.team]) teamsKanbanMap[r.team] = [];
