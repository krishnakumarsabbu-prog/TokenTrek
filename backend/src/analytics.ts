...
import { computeUnifiedDeveloperScores } from './scoringEngine';
...
export function computeAIWaste() {
  ...
  const items = store.waste_items.map((w, index) => {
  ...
  const trendData: { date: string; waste: number }[] = [];
  const dailyStats = store.daily_stats || [];
...
export function computeDeveloperXP() {
  return computeUnifiedDeveloperScores().map(d => ({
