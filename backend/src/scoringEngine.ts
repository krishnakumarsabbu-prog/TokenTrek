...
// ── Core score calculation for a single developer ─────────────────────────────
...
export interface DeveloperScoreInput {
  ...
  avatar: string;
  teamId: number;
  /** From store.developer_scores (may be undefined for Devin-only devs) */
  existingScore?: number;
  existingTrend?: number;
}

export interface UnifiedDeveloperScore {
  developer_id: number;
  name: string;
  avatar: string;
  team: string;
  ...
  // Sub-components (0–99 each)
  devinScore: number;       // Devin contribution
  gitScore: number;         // Git / prompt contribution
  efficiencyScore: number;  // AI usage efficiency
  qualityScore: number;     // Quality impact

  // Raw metrics from Devin
  devinSessions: number;
  mergedPRs: number;
  totalPRs: number;
  acuUsed: number;
  categories: string[];
  ...
  totalScore2: number;      // alias kept for league compat
 ...
 */
export function computeUnifiedDeveloperScores(): UnifiedDeveloperScore[] {
  const developers = store.developers;
  const teams = store.teams;
  const platforms = store.platforms;

  const results: UnifiedDeveloperScore[] = developers.map((dev, i) => {
    ...
    const devinStat = store.devin_developer_stats.find(
      d => d.user_email.toLowerCase() === ((dev as any).email || '').toLowerCase() ||
    ...
    const maxMerged = Math.max(...store.devin_developer_stats.map(d => d.merged_prs), 1);
    const maxSessions = Math.max(...store.devin_developer_stats.map(d => d.sessions), 1);
    ...
    const gitContrib = clamp(
      Math.round(baseScore - seededRand(nameSeed * 7) * 8 + 3),
      50, 99
    );
    ...
    const qualityImpact = clamp(
      devinSessions > 0
        ? Math.round(prSuccessRate * 0.7 + (successfulPrompts / Math.max(promptsUsed, 1)) * 100 * 0.3)
        : clamp(Math.round(baseScore * 0.9 + seededRand(nameSeed + 5) * 10), 50, 99),
      50, 99
    );
...
export interface UnifiedTeamScore {
  ...
  weeklyChange: number;
  devinSessions: number;
  devinMergedPRs: number;
  devinAiScore: number;
}

export function computeUnifiedTeamScores(): UnifiedTeamScore[] {
  const teams = store.teams;
  const devScores = computeUnifiedDeveloperScores();

  const results: UnifiedTeamScore[] = teams.map((team, i) => {
    ...
    const devinTeam = store.devin_team_stats.find(dt =>
      dt.team_name.toLowerCase() === team.name.toLowerCase()
    );
