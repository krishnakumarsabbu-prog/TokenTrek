...
exports.computeDailyUsage = computeDailyUsage;
exports.computePlatformUsage = computePlatformUsage;
exports.computeDeveloperScores = computeDeveloperScores;
exports.computePromptRanking = computePromptRanking;
exports.computeTeamRanking = computeTeamRanking;
exports.computeModelEfficiency = computeModelEfficiency;
exports.computeAIWaste = computeAIWaste;
exports.computeRecommendations = computeRecommendations;
exports.computeDeveloperXP = computeDeveloperXP;
exports.computeAIInsights = computeAIInsights;
exports.computeFullReport = computeFullReport;
...
function computeDailyUsage() {
    const byDate = {};
    for (const r of db_1.store.daily_stats) {
        if (!byDate[r.date])
            byDate[r.date] = { requests: 0, tokens: 0, cost: 0 };
...
// ─── Platform Usage ────────────────────────────────────────────────────────
function computePlatformUsage() {
    const byPlatform = {};
    for (const r of db_1.store.daily_stats) {
        if (!byPlatform[r.platform_id])
            byPlatform[r.platform_id] = { requests: 0, tokens: 0, cost: 0 };
    ...
    return Object.entries(byPlatform)
        .map(([pid, d]) => {
        const platform = db_1.store.platforms.find(p => p.id === Number(pid));
        return {
...
// ─── Developer Scores ──────────────────────────────────────────────────────
function computeDeveloperScores() {
    return db_1.store.developer_scores
        .map(ds => {
        const dev = db_1.store.developers.find(d => d.id === ds.developer_id);
        const team = db_1.store.teams.find(t => t.id === dev?.team_id);
        return {
...
function computeTeamRanking() {
    const totalCost = db_1.store.team_costs.reduce((s, t) => s + t.cost, 0);
    return db_1.store.team_costs
        .map(tc => {
...
function computeModelEfficiency() {
    const totalCost = db_1.store.model_costs.reduce((s, m) => s + m.cost, 0);
    return db_1.store.model_costs
        .map(m => {
...
// ─── Developer XP System — delegates to unified scoring engine ───────────────
function computeDeveloperXP() {
    return (0, scoringEngine_1.computeUnifiedDeveloperScores)().map(d => ({
        ...
        score: d.totalScore,
        trend: d.weeklyChange,
        aiEfficiency: d.aiEfficiency,
        promptsUsed: d.promptsUsed,
        successfulPrompts: d.successfulPrompts,
        acceptedCode: d.acceptedCode,
        timeSavedHrs: d.timeSavedHrs,
...
function computeAIInsights() {
    ...
    if (cursorPlatform && githubPlatform) {
        const cursorDevs = db_1.store.developers.filter(d => db_1.store.live_activity.some(la => la.developer_id === d.id && la.platform_id === cursorPlatform.platform_id));
        const cursorDevScores = devScores.filter(ds => cursorDevs.some(cd => cd.name === ds.developer));
    ...
    const weekendStats = db_1.store.daily_stats.filter(s => [0, 6].includes(new Date(s.date).getDay()));
    const weekdayStats = db_1.store.daily_stats.filter(s => ![0, 6].includes(new Date(s.date).getDay()));
