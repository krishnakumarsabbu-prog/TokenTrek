...
exports.recalculateFromDevin = recalculateFromDevin;
...
function recalculateFromDevin() {
    ...
    if (!db_1.store.platforms.find(p => p.id === DEVIN_PLATFORM_ID)) {
        db_1.store.platforms.push(DEVIN_PLATFORM);
    }
    ...
    for (const s of sessions) {
        ...
        if (s.category)
            acc.categories.add(s.category);
        acc.dates.push(s.created_at.slice(0, 10));
        if (s.created_at > acc.lastActivity)
            acc.lastActivity = s.created_at;
    }
    ...
    let nextTeamId = Math.max(0, ...db_1.store.teams.map(t => t.id)) + 1;
    // Index existing teams by name for reuse
    const existingTeamsByName = new Map(db_1.store.teams.map(t => [t.name.toLowerCase(), t]));
    ...
    for (const [, ta] of teamMap) {
        ...
        if (existing) {
            ...
            const newTeam = { id: nextTeamId++, name: ta.name };
            db_1.store.teams.push(newTeam);
            teamIdByName.set(ta.name, newTeam.id);
        }
    }
    // ── 5. Upsert Developers into store ──────────────────────────────────────
    let nextDevId = Math.max(0, ...db_1.store.developers.map(d => d.id)) + 1;
    const existingDevsByEmail = new Map(db_1.store.developers
        .filter(d => d.email)
        .map(d => [d.email.toLowerCase(), d]));
    const devIdByEmail = new Map();
    for (const [, acc] of devMap) {
        ...
        if (existing) {
            ...
            db_1.store.developers.push(newDev);
    ...
    db_1.store.developer_scores = db_1.store.developer_scores.filter(ds => !ds.source_devin);
    let scoreIdx = 0;
    ...
    for (const acc of devAccList) {
        ...
        const rawScore = Math.round(successRate * 0.4 +
            acuEfficiency * 0.3 +
            sessionBonus * 0.2 +
            diversityBonus * 0.1 +
        ...
        db_1.store.developer_scores.push(entry);
    }
    // ── 7. Upsert TeamCosts ────────────────────────────────────────────────────
    db_1.store.team_costs = db_1.store.team_costs.filter(tc => !tc.source_devin);
    let tcIdx = 0;
    for (const [, ta] of teamMap) {
        ...
        db_1.store.team_costs.push(tc);
    }
    // ── 8. Synthesize DailyStats from Devin sessions ──────────────────────────
    // Remove existing Devin-sourced daily stats
    db_1.store.daily_stats = db_1.store.daily_stats.filter(ds => !ds.source_devin);
    const dateAcu = new Map();
    ...
    let dsIdx = 0;
    const maxExistingDsId = db_1.store.daily_stats.length > 0 ? Math.max(...db_1.store.daily_stats.map(d => d.id)) : 0;
    for (const [date, acu] of dateAcu) {
        ...
        db_1.store.daily_stats.push(ds);
    }
    // ── 9. Synthesize LiveActivity from recent Devin sessions ────────────────
    db_1.store.live_activity = db_1.store.live_activity.filter(la => !la.source_devin);
    const recentSessions = [...sessions]
        .sort((a, b) => b.created_at.localeCompare(a.created_at))
        .slice(0, 20);
    let laIdx = 0;
    const maxLaId = db_1.store.live_activity.length > 0 ? Math.max(...db_1.store.live_activity.map(la => la.id)) : 0;
    for (const s of recentSessions) {
        ...
        db_1.store.live_activity.push(la);
    }
    // ── 10. Synthesize ModelCosts ──────────────────────────────────────────────
    db_1.store.model_costs = db_1.store.model_costs.filter(mc => !mc.source_devin);
    const totalAcu = sessions.reduce((s, x) => s + x.acu_used, 0);
    const devinCost = totalAcu * 0.2;
    if (devinCost > 0) {
        const totalExisting = db_1.store.model_costs.reduce((s, m) => s + m.cost, 0);
        ...
        db_1.store.model_costs.push(mc);
    }
    // ── 11. Add Devin-specific insights ──────────────────────────────────────
    db_1.store.insights = db_1.store.insights.filter(i => !i.source_devin);
