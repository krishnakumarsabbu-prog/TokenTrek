...
function seed() {
    ...
    const insertDev = db.prepare('INSERT INTO developers (name, avatar, team_id) VALUES (?, ?, (SELECT id FROM teams WHERE name = ?))');
    ...
    const insertStats = db.prepare('INSERT INTO daily_stats (date, platform_id, requests, tokens, cost) VALUES (?, (SELECT id FROM platforms WHERE name = ?), ?, ?, ?)');
    ...
    const insertScore = db.prepare('INSERT INTO developer_scores (developer_id, score, trend, period) VALUES ((SELECT id FROM developers WHERE name = ?), ?, ?, ?)');
    ...
    const insertTeamCost = db.prepare('INSERT INTO team_costs (team_id, cost, change_pct, period) VALUES ((SELECT id FROM teams WHERE name = ?), ?, ?, ?)');
    ...
    const insertActivity = db.prepare(`INSERT INTO live_activity (developer_id, action, platform_id, created_at)
