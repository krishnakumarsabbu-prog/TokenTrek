...
import { Activity, TrendingUp, DollarSign, Users, Clock, Zap, ArrowUpRight, ArrowDownRight, ChevronRight, BarChart2 } from 'lucide-react';
import {
...
import {
  KpiCard, SectionCard, Avatar, ProgressBar, Badge, CardSkeleton, ChartSkeleton,
  Trend, PageHeader, LivePulse, ChartContainer,
} from '../components/ui';
...
function ActivitySkeleton() {
  return (
    <div className="space-y-0">
      {[1, 2, 3, 4, 5].map(i => (
        <div key={i} className="flex items-center gap-3 px-4 py-3.5 border-b" style={{ borderColor: '#f0f4f8' }}>
          <div className="w-8 h-8 rounded-full flex-shrink-0 overflow-hidden relative" style={{ background: '#edf1f5' }}>
            <motion.div
          ...
          <div className="flex-1 space-y-1.5">
            <div className="h-3 rounded-md w-3/4 overflow-hidden relative" style={{ background: '#edf1f5' }}>
              <motion.div
                className="absolute inset-0"
                style={{ background: 'linear-gradient(90deg, transparent, rgba(255,255,255,0.6), transparent)' }}
                animate={{ x: ['-100%', '100%'] }}
                transition={{ duration: 1.4, repeat: Infinity, ease: 'linear', delay: i * 0.15 + 0.1 }}
              />
            </div>
            <div className="h-2.5 rounded-md w-1/2 overflow-hidden relative" style={{ background: '#edf1f5' }}>
              <motion.div
                className="absolute inset-0"
                style={{ background: 'linear-gradient(90deg, transparent, rgba(255,255,255,0.6), transparent)' }}
                animate={{ x: ['-100%', '100%'] }}
                transition={{ duration: 1.4, repeat: Infinity, ease: 'linear', delay: i * 0.15 + 0.2 }}
              />
            </div>
          </div>
          <div className="h-3 rounded-md w-10 overflow-hidden relative" style={{ background: '#edf1f5' }}>
            <motion.div
              className="absolute inset-0"
              style={{ background: 'linear-gradient(90deg, transparent, rgba(255,255,255,0.6), transparent)' }}
              animate={{ x: ['-100%', '100%'] }}
              transition={{ duration: 1.4, repeat: Infinity, ease: 'linear', delay: i * 0.15 + 0.3 }}
            />
          </div>
        </div>
      ))}
          ...
          >
            {stats.isLoading
              ? Array.from({ length: 6 }).map((_, i) => (
                  <motion.div key={i} variants={staggerItem}><CardSkeleton /></motion.div>
                ))
              : (
                <>
                ...
                <div className="p-4">
                  ...
                  >
                    {(platformCosts.data?.items || []).map((item: any, i: number) => (
                      <motion.div key={i} className="flex items-center justify-between text-xs" variants={staggerItem}>
                        <div className="flex items-center gap-2">
                ...
                <div className="p-4 space-y-3">
                  {[1,2,3,4,5].map(i => (
                    <div key={i} className="flex gap-3">
                      <div className="w-8 h-8 rounded-full overflow-hidden relative" style={{ background: '#edf1f5' }}>
                        <motion.div
                          className="absolute inset-0"
                          style={{ background: 'linear-gradient(90deg, transparent, rgba(255,255,255,0.6), transparent)' }}
                          animate={{ x: ['-100%', '100%'] }}
                      ...
                      <div className="flex-1 space-y-1.5">
                        <div className="h-3 rounded w-3/4 overflow-hidden relative" style={{ background: '#edf1f5' }}>
                          <motion.div
                            className="absolute inset-0"
                            style={{ background: 'linear-gradient(90deg, transparent, rgba(255,255,255,0.6), transparent)' }}
                            animate={{ x: ['-100%', '100%'] }}
                            transition={{ duration: 1.4, repeat: Infinity, ease: 'linear', delay: i * 0.1 + 0.1 }}
                          />
                        </div>
                        <div className="h-2 rounded w-full overflow-hidden relative" style={{ background: '#edf1f5' }}>
                          <motion.div
                            className="absolute inset-0"
                            style={{ background: 'linear-gradient(90deg, transparent, rgba(255,255,255,0.6), transparent)' }}
                            animate={{ x: ['-100%', '100%'] }}
                            transition={{ duration: 1.4, repeat: Infinity, ease: 'linear', delay: i * 0.1 + 0.2 }}
                          />
                        </div>
                      </div>
                    </div>
                  ...
                  {[1,2,3,4,5].map(i => (
                    <div key={i} className="flex items-center justify-between px-5 py-3">
                      <div className="flex items-center gap-2.5">
                        <div className="w-7 h-7 rounded-lg overflow-hidden relative" style={{ background: '#edf1f5' }}>
                          <motion.div
                            className="absolute inset-0"
                            style={{ background: 'linear-gradient(90deg, transparent, rgba(255,255,255,0.6), transparent)' }}
                            animate={{ x: ['-100%', '100%'] }}
                        ...
                        <div className="h-3 w-24 rounded overflow-hidden relative" style={{ background: '#edf1f5' }}>
                          <motion.div
                            className="absolute inset-0"
                            style={{ background: 'linear-gradient(90deg, transparent, rgba(255,255,255,0.6), transparent)' }}
                            animate={{ x: ['-100%', '100%'] }}
                      ...
                      <div className="h-4 w-16 rounded overflow-hidden relative" style={{ background: '#edf1f5' }}>
                        <motion.div
                          className="absolute inset-0"
                          style={{ background: 'linear-gradient(90deg, transparent, rgba(255,255,255,0.6), transparent)' }}
                          animate={{ x: ['-100%', '100%'] }}
                ...
                <div className="divide-y" style={{ borderColor: '#f0f4f8' }}>
                  {[1,2,3,4,5].map(i => (
                    <div key={i} className="px-5 py-3">
                      <div className="h-3 rounded w-4/5 mb-1.5 overflow-hidden relative" style={{ background: '#edf1f5' }}>
                        <motion.div
                          className="absolute inset-0"
                          style={{ background: 'linear-gradient(90deg, transparent, rgba(255,255,255,0.6), transparent)' }}
                          animate={{ x: ['-100%', '100%'] }}
                      ...
                      <div className="h-2.5 rounded w-2/5 overflow-hidden relative" style={{ background: '#edf1f5' }}>
                        <motion.div
                          className="absolute inset-0"
                          style={{ background: 'linear-gradient(90deg, transparent, rgba(255,255,255,0.6), transparent)' }}
                          animate={{ x: ['-100%', '100%'] }}
              ...
              <SectionCard title="AI Waste Detected" action={
                ...
                {wasteItems.isLoading ? (
                  <div className="p-4 space-y-2">
                    {[1,2,3].map(i => (
                      <div key={i} className="h-8 rounded-lg overflow-hidden relative" style={{ background: '#edf1f5' }}>
                        <motion.div
                          className="absolute inset-0"
                          style={{ background: 'linear-gradient(90deg, transparent, rgba(255,255,255,0.6), transparent)' }}
                          animate={{ x: ['-100%', '100%'] }}
                  ...
                  >
                    {(wasteItems.data || []).map((w: any, i: number) => (
                      <motion.div
                        key={i}
                        className="flex items-center justify-between px-5 py-2.5 cursor-default"
                        variants={staggerItem}
                        whileHover={{ background: '#f7fafd' }}
                        transition={{ duration: 0.15 }}
                      >
              ...
              <SectionCard title="AI Insights" action={
                ...
                {insights.isLoading ? (
                  <div className="p-3 space-y-2">
                    {[1,2].map(i => (
                      <div key={i} className="h-14 rounded-xl overflow-hidden relative" style={{ background: '#edf1f5' }}>
                        <motion.div
                          className="absolute inset-0"
                          style={{ background: 'linear-gradient(90deg, transparent, rgba(255,255,255,0.6), transparent)' }}
                          animate={{ x: ['-100%', '100%'] }}
                    ...
                    {(insights.data || []).slice(0, 3).map((ins: any, i: number) => {
                      ...
                      return (
                        <motion.div
                          ...
                          variants={staggerItem}
                          whileHover={{ scale: 1.01, filter: 'brightness(0.97)' }}
                          transition={{ duration: 0.15 }}
                        >
                          <div
      ...
      >
            ...
            >
              <AnimatePresence>
                {(liveActivity.data?.items || []).map((a: any, i: number) => (
                  <motion.div
                    key={`${a.developer}-${i}`}
                    className="flex items-start gap-3 px-4 py-3.5 border-b cursor-default"
                    style={{ borderColor: '#f0f4f8' }}
                    variants={staggerItem}
                    whileHover={{ background: '#f7fafd' }}
                    layout
        ...
        >
          ...
          <div className="pt-2 border-t" style={{ borderColor: '#f0f4f8' }}>
            ...
            {Object.entries(PLATFORM_EVENT_COLORS).map(([name, color], idx) => {
              const pct = [72, 45, 88, 33, 61][idx] ?? 50;
              return (
                <div key={name} className="flex items-center gap-2 mb-1.5">
                  ...
                  <div className="flex-1 h-1 rounded-full overflow-hidden" style={{ background: '#f0f4f8' }}>
                    <motion.div
                      ...
                      initial={{ width: 0 }}
                      animate={{ width: `${pct}%` }}
                      transition={{ duration: 1, ease: [0.22, 1, 0.36, 1], delay: 0.6 + idx * 0.1 }}
                    />
                  </div>
